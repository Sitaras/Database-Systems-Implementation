...
unsigned int hash_HT(char id[],int buckets){ //find the hash table index for the given id
...
int HT_InsertEntry( HT_info header_info, Record record) {
        ...
        if(header_info.keyType=='i'){   // check if the key is int type
          ...
          index=hash_HT(convertToInt,header_info.buckets); // call the hash function so as to return the index that the new record will be stored
        }
...
int HT_GetAllEntries( HT_info header_info, void *value){
        ...
        if(header_info.keyType=='i'){   // check if the key is int type
          ...
          index=hash_HT(convertToInt,header_info.buckets); // call the hash function so as to return the index that record we want is in
        }
        // else if(header_info.keyType=='c'){   // in case the key is char type (isn't used in project1)
        //   index=hash((char *)value,header_info.buckets);
        // }
...
int HT_HashStatistics( char* filename ) {
        ...
        if (BF_ReadBlock(fd,headerBlockId, &block) < 0) {    // read the header block in order to find from the pointer in which block(s) the hash table is stored
                BF_PrintError("Error getting block");
                return -1;
        }
        block+=sizeof(HT_info);   // skip the HT_info
        ...
        printf("________ PRIMARY HASH STATISTICS ________\n");
